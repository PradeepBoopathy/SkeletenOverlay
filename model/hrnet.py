...
class HRNet(nn.Module):
    def __init__(self, base_channels=32, out_channels=17):
        ...
        self.transition1 = self.make_transition1(inplanes=256, branch1_planes=self.base_channels,
                                                 branch2_planes=self.base_channels * 2)
        self.stage2 = self.make_stage2(base_channels=self.base_channels)
        self.transition2 = self.make_transition2(branch1_inplanes=None, branch2_inplanes=None,
                                                 branch3_inplanes=self.base_channels * 2,
                                                 branch1_planes=None, branch2_planes=None,
                                                 branch3_planes=self.base_channels * 4)
        self.stage3 = self.make_stage3(base_channels=self.base_channels)
        self.transition3 = self.make_transition3(branch1_inplanes=None, branch2_inplanes=None, branch3_inplanes=None,
                                                 branch4_inplanes=self.base_channels * 4,
                                                 branch1_planes=None, branch2_planes=None, branch3_planes=None,
                                                 branch4_planes=self.base_channels * 8)
    ...
    def make_stem(self, inplanes=3, planes=64, momentum=0.1):
        return nn.Sequential(
            nn.Conv2d(in_channels=inplanes, out_channels=planes, kernel_size=3, stride=2, padding=1, bias=False),
    ...
    def make_transition1(self, inplanes=256, branch1_planes=32, branch2_planes=64, branch1_stride=1, branch2_stride=2,
                         momentum=0.1):
        return nn.ModuleList([nn.Sequential(
            nn.Conv2d(inplanes, branch1_planes, kernel_size=3, stride=branch1_stride, padding=1, bias=False),
            nn.BatchNorm2d(branch1_planes, momentum=momentum),
            nn.ReLU(inplace=True)),
                              nn.Sequential(
                                  nn.Conv2d(inplanes, branch2_planes, kernel_size=3, stride=branch2_stride, padding=1,
                                            bias=False),
    ...
    def make_transition2(self,
        ...
        return nn.ModuleList([nn.Identity(),
                              nn.Identity(),
                              nn.Sequential(
                                  nn.Conv2d(branch3_inplanes, branch3_planes, kernel_size=3, stride=branch3_stride,
    ...
    def make_transition3(self,
        ...
        return nn.ModuleList([nn.Identity(),
                              ...
                              nn.Identity(),
                              nn.Sequential(
                                  nn.Conv2d(branch4_inplanes, branch4_planes, kernel_size=3, stride=branch4_stride,
                                            padding=1, bias=False),
