...
def train(model, train_dataset, train_loader, criterion, optimizer, epoch, print_freq):
    ...
    for i, (input, targets, target_weights, data_idx, flipped) in enumerate(train_loader):
        ...
        if (i + 1) % print_freq == 0:
            print("training... [{} / {}]( loss : {}, accuracy : {})".format(len(train_loader), i + 1, loss.item(),
                                                                            avg_acc))
...
def val(model, val_dataset, val_loader, criterion, epoch, print_freq):
    ...
    with torch.no_grad():
        for i, (input, targets, target_weights, data_idx, flipped) in enumerate(val_loader):
            ...
            if (i + 1) % print_freq == 0:
                print("validating... [{} / {}]( loss : {}, accuracy : {})".format(len(val_loader), i + 1, loss.item(),
                                                                                  avg_acc))
...
train_annopath = "data/coco/annotations/coco_wholebody_train_v1.0.json"
train_imagepath = "data/coco/train2017"
val_annopath = "data/coco/annotations/coco_wholebody_val_v1.0.json"
val_imagepath = "data/coco/val2017"
...
train_dataset = COCODataset(train_annopath, train_imagepath,
...
val_dataset = COCODataset(val_annopath, val_imagepath,
                          transforms=transforms.Compose([transforms.ToTensor(), normalize]),
                          image_height=384, image_width=288, heatmap_height=96, heatmap_width=72)
...
if __name__ == "__main__":
    ...
    for epoch in range(model_dict['epoch'], 5000):
        ...
        print("best train loss : {}, best train accuracy : {})".format(model_dict['best_train_loss'],
                                                                       model_dict['best_train_accuracy']))
        ...
        print("best val loss : {}, best val accuracy : {})".format(model_dict['best_val_loss'],
                                                                   model_dict['best_val_accuracy']))
