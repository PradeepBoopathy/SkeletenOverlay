...
class COCODataset(Dataset):
    def __init__(self, anno_path, image_root_path,
        ...
        for raw_image_id in self.raw_image_ids:
            ...
            for person_anno in people_anno:
                ...
                clean_bbox_topleft = (
                center[0] - self.image_width / 2 * scale, center[1] - self.image_height / 2 * scale)
                clean_bbox = [clean_bbox_topleft[0], clean_bbox_topleft[1], self.image_width * scale,
                ...
                keypoints = np.array([*body_keypoints,
                                      ...
                                      *feet_keypoints[2], *feet_keypoints[5]], dtype=np.float32)

                keypoints = keypoints.reshape((-1, 3))

                joints = keypoints.copy()
                joints[:, 2] = np.zeros(self.num_joints, dtype=np.float32)

                joint_vis = np.zeros((self.num_joints, 3), dtype=np.float32)
    ...
    def get_affine(self, bbox):
        p1 = np.float32([[int(bbox[0]), int(bbox[1])], [int(bbox[0] + bbox[2]), int(bbox[1])],
    ...
    def generate_heatmap_from_joints(self, joints, joint_vis, sigma=2, use_different_joints_weight=False):
        target_weights = np.ones((self.num_joints, 1), dtype=np.float32)
